...
const FRAME_EXPLICIT_SELF_MODIFIERS = /^(?:(?:刚才|刚刚|刚|这会儿|今天|昨天|前天|今早|昨晚|今晚|早上|上午|中午|下午|晚上|夜里|早餐|午餐|晚餐|午饭|晚饭|在公司|然后|接着|后来|又|没|没有)\s*)*$/u;
...
export function resolvePredicateFrameSubject(
  ...
  const explicitSelf = /^(?:是\s*)?我(?:\s*(?:自己|本人))?/u.exec(prefix);
