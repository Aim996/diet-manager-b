...
function applyDailyProgressDelta(
  current: FrozenDailyProgress,
  before: FrozenDailyProgress,
  after: FrozenDailyProgress,
): FrozenDailyProgress {
  if (
    current.date !== before.date || current.date !== after.date ||
...
function zeroDailyProgress(date: string, timezone: "Asia/Shanghai"): FrozenDailyProgress {
  return Object.freeze({
    date,
    timezone,
...
function freezeCorrectionDailyProgress(
  input: FrozenInput,
  envelopeId: string,
  idempotencyKey: string,
): FrozenInput {
  ...
  const execution = plainRecord(
    input.payload,
    ["envelope_id", "input_digest", "items", "payload", "status"],
    "correction_progress_execution",
  );
  if (
    execution.envelope_id !== envelopeId ||
  ...
  if (
    ...
    const domainPayload = plainRecord(
      execution.payload,
      ["authority_kind", "inventory_location_correction"],
      "location_correction_payload",
    );
    ...
    const eventRows = input.database.prepare(
      `SELECT event_id, operation_id, event_type, fact_kind, payload_json
       FROM event_records WHERE envelope_id = ?`,
    ).all(envelopeId) as Array<{
      event_id: string;
      operation_id: string;
      event_type: string;
    ...
    if (
      eventRows.length !== 1 || !event || event.event_type !== "inventory_adjusted" ||
    ...
    if (
      fact.result.operation_id !== event.operation_id ||
    ...
    const outboxes = input.database.prepare(
      `SELECT effect_kind, state FROM effect_outbox WHERE envelope_id = ? AND operation_id = ?`,
    ).all(envelopeId, event.operation_id) as Array<{ effect_kind: string; state: string }>;
    const projection = input.database.prepare(
      `SELECT last_event_id, payload_json FROM inventory_batch_projections WHERE batch_id = ?`,
    ).get(fact.batch_id) as { last_event_id: string; payload_json: string } | undefined;
    if (
      outboxes.length !== 1 || outboxes[0]?.effect_kind !== "inventory_location_correction" ||
  ...
  const domainPayload = plainRecord(
    execution.payload,
    ["authority_kind", "daily_progress", "daily_progress_by_date"],
    "correction_progress_payload",
  );
  if (domainPayload.authority_kind !== "diet-manager/domain-execution/v1") {
    return authorityInvalid("correction_progress_payload");
  }
  const replacement = parseDailyProgress(
    domainPayload.daily_progress,
    "correction_progress_replacement",
  );
  if (
    !Array.isArray(domainPayload.daily_progress_by_date) ||
  ...
  const replacementsByDate = Object.freeze(
    domainPayload.daily_progress_by_date.map((candidate) =>
      parseDailyProgress(candidate, "correction_progress_by_date")),
  );
  ...
  if (bundleRows.length !== 1) return authorityInvalid("correction_progress_bundle");
  ...
  if (
    ...
    outboxes.some((outbox) =>
      outbox.effect_kind !== "daily_progress_replacement" &&
      outbox.effect_kind !== "correction_inventory_compensation")
  ) {
    return authorityInvalid("correction_progress_bundle");
  }
  const boundReplacements: FrozenDailyProgress[] = [];
  const boundBefores: FrozenDailyProgress[] = [];
  const boundAfters: FrozenDailyProgress[] = [];
  for (let index = 0; index < outboxes.length; index += 1) {
    const outbox = outboxes[index];
    ...
    const effect = plainRecord(
      ...
      progress
        ...
        : ["effect_id", "state"],
      "correction_progress_effect",
    );
    if (
      effect.effect_id !== outbox.effect_id || effect.state !== outbox.state ||
      (outbox.state !== "succeeded" && outbox.state !== "permanent_business_skip")
    ) return authorityInvalid("correction_progress_bundle");
    if (progress) {
  ...
  const boundByDate = new Map<string, {
    readonly replacement: FrozenDailyProgress;
    readonly before: FrozenDailyProgress;
    readonly after: FrozenDailyProgress;
  }>();
  ...
  const alignedReplacements: FrozenDailyProgress[] = [];
  const alignedBefores: FrozenDailyProgress[] = [];
  const alignedAfters: FrozenDailyProgress[] = [];
  for (const byDate of replacementsByDate) {
    const bound = boundByDate.get(byDate.date);
    if (bound === undefined) return authorityInvalid("correction_progress_bundle");
    alignedReplacements.push(bound.replacement);
    alignedBefores.push(bound.before);
    alignedAfters.push(bound.after);
  }
  if (
    alignedReplacements.length !== replacementsByDate.length ||
  ...
  for (let index = 0; index < alignedReplacements.length; index += 1) {
    ...
    const previousRow = input.database.prepare(
      `SELECT generated_at, payload_json FROM daily_progress_snapshots
       WHERE date = ? AND timezone = ? ORDER BY generated_at DESC, progress_snapshot_id DESC LIMIT 1`,
    ).get(boundReplacement.date, boundReplacement.timezone) as {
      generated_at: string;
      payload_json: string;
    } | undefined;
    let previous: FrozenDailyProgress;
    let previousGeneratedAt: string | null;
    if (previousRow === undefined) {
      ...
      try {
        previousValue = JSON.parse(previousRow.payload_json) as unknown;
      } catch {
  ...
  if (
    canonicalJson(item.daily_progress) !== canonicalJson(replacement) ||
...
function assertInventoryLocationCorrectionTerminalAuthority(
  input: FrozenInput,
  envelopeId: string,
): void {
  if (
  ...
  const events = input.database.prepare(
    `SELECT event_id, operation_id FROM event_records
     WHERE envelope_id = ? AND event_type = 'inventory_adjusted' AND fact_kind = 'inventory'`,
  ).all(envelopeId) as Array<{ event_id: string; operation_id: string }>;
  const event = events[0];
  if (events.length !== 1 || !event) return authorityInvalid("location_correction_event");
  try {
    assertAppliedInventoryLocationCorrectionAuthority(
      input.database,
      envelopeId,
...
export function finalizeEnvelope(
  ...
  try {
    ...
    if (authority.envelope_state === "finalized") {
      assertInventoryLocationCorrectionTerminalAuthority(
        frozen,
