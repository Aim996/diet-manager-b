...
状态：**历史报告。** 0.1.0 主链、构建产物和本地安装入口当时已完成；该工程快照现冻结于 `releases/v0.1`。本报告中的就绪表述只属于 0.1.0 候选。当前目标版本为 0.1.1（见 `PRODUCT-0.1.1-ledger.json`）；阶段 1 的追加式撤销（`undo_record`）仍是发布阻断项，完整 0.1 尚未全部交付。
