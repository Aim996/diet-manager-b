...
const DIRECT_OR_COORDINATED_WATER = /(?:^|和|与|、)\s*(?:([0-9]+)\s*(?:ml|毫升)\s*)?(矿泉水|白水|水)(?=$|[\s,，。；;！!？?、和与又吗么嘛呢时"'”’」』》】）)\]}])/gu;
const NATURAL_UNIT_WATER = /(?:^|和|与|、)\s*(?:[0-9]+|[一二两三四五六七八九十]+)\s*(?:杯|口|瓶)\s*(?:矿泉水|白水|水)(?=$|[\s,，。；;！!？?、和与又吗么嘛呢时"'”’」』》】）)\]}])/gu;
const ADJUNCT_START = /(?:时|后)?(?:看见|看到|拿着|放着|旁边|桌上|还有|使用|用了)/u;
const PUNCTUATED_WATER_CONTINUATION = /^\s*[，,]\s*([0-9]+)\s*(?:ml|毫升)\s*(矿泉水|白水|水)(?=$|[\s,，。；;！!？?、和与又吗么嘛呢"'”’」』》】）)\]}])/u;
...
export interface WaterFrameScan {
  ...
  readonly self_unquantified_count: number;
  ...
  for (const frame of parseIngestionPredicateFrames(sourceText)) {
    ...
    for (const direct of directText.matchAll(DIRECT_OR_COORDINATED_WATER)) {
      ...
      if (quantityText === undefined) {
        if (subject.disposition === "resolved") selfUnquantifiedCount += 1;
        continue;
      }
