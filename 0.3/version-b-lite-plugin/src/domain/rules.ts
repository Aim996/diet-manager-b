...
export function resolveInventoryMatch(
  ...
  if (input.observed_microunits === null) {
    if (
      input.nutrition_adoption_microunits !== null ||
...
function freezeNutritionSelection(
  ...
  if (unknown) {
    ...
    ].includes(String(value.basis_kind))) return invalid("nutrition_basis_kind");
...
function scaleKnown(
  value: number | null,
  adoptedMicrounits: number,
  basisMicrounits: number,
): number | null {
  ...
  if (scaled > BigInt(Number.MAX_SAFE_INTEGER)) return invalid("nutrition_scaled");
  return Number(scaled);
}

export function scaleNutritionVector(
  value: NutritionVector,
  adoptedMicrounits: number,
  basisMicrounits: number,
): NutritionVector {
