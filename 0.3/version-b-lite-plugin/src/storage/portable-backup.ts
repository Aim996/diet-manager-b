...
import {
  ...
  installAuthoritySecret,
} from "../application/core-runtime.js";
import {
...
interface PortableBackupEnvelopeV1 extends PortableBackupHeaderV1 {
  readonly encryption: Readonly<{
    algorithm: "aes-256-gcm";
    iv_base64: string;
    tag_base64: string;
...
interface PortableBackupPayloadV1 {
  readonly database_base64: string;
  readonly database_sha256: string;
  readonly authority_secret_base64: string;
}
...
function validatePassphrase(value: unknown): Uint8Array {
  if (!(value instanceof Uint8Array) || isProxy(value)) return invalid("passphrase");
...
function validateSha256(value: unknown): string {
  if (typeof value !== "string" || !/^[A-F0-9]{64}$/u.test(value)) return invalid("expected_sha256");
  return value;
}

function readAuthoritySecret(root: string): Uint8Array {
  const path = join(root, CORE_RUNTIME_SECRET_FILENAME);
  const stat = ordinaryFile(path, "authority_secret");
  if (stat.size !== 32n) return invalid("authority_secret");
...
function serializeHeader(header: PortableBackupHeaderV1): string {
...
function serializeEnvelope(envelope: PortableBackupEnvelopeV1): string {
  return JSON.stringify({
...
function serializePayload(payload: PortableBackupPayloadV1): string {
  return JSON.stringify({
...
function deriveKey(passphrase: Uint8Array, salt: Uint8Array): Buffer {
  return scryptSync(passphrase, salt, KEY_BYTES, {
    N: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P, maxmem: SCRYPT_MAXMEM,
  });
}

function parseEnvelope(text: string): Readonly<PortableBackupEnvelopeV1> {
  let parsed: unknown;
  try {
  ...
  const encryption = envelope.encryption as Record<string, unknown> | undefined;
  ...
  const result: PortableBackupEnvelopeV1 = {
    ...
    kdf: Object.freeze({
      ...
      r: 8 as const,
      p: 1 as const,
      salt_base64: kdf.salt_base64 as string,
    }),
    encryption: Object.freeze({
      algorithm: "aes-256-gcm" as const,
      iv_base64: encryption.iv_base64 as string,
      tag_base64: encryption.tag_base64 as string,
    }),
    ciphertext_base64: envelope.ciphertext_base64 as string,
  };
  // 拒绝非规范（重排/美化）JSON：磁盘字节必须是上面固定键序的逐字序列化。
  if (serializeEnvelope(result) !== text.trim()) return invalid("envelope");
  return Object.freeze(result);
}

function decryptPayload(
  envelope: Readonly<PortableBackupEnvelopeV1>,
  passphrase: Uint8Array,
): Readonly<PortableBackupPayloadV1> {
  ...
  try {
    ...
    if (error instanceof Error && /^DIET_PORTABLE_BACKUP_INVALID:/.test(error.message)) throw error;
...
export async function createPortableBackup(
  value: Readonly<CreatePortableBackupInput>,
): Promise<Readonly<PortableBackupResult>> {
  ...
  try {
    ...
    let key: Buffer | undefined;
    let payload: Readonly<PortableBackupPayloadV1> | undefined;
    try {
      key = deriveKey(passphrase, salt);
      const header: PortableBackupHeaderV1 = {
        format: PORTABLE_BACKUP_FORMAT,
      ...
      const envelope: PortableBackupEnvelopeV1 = {
        ...header,
        encryption: Object.freeze({
          algorithm: "aes-256-gcm",
          iv_base64: header.encryption.iv_base64,
      ...
      if (roundTrip.database_sha256 !== payload.database_sha256) return invalid("database_sha256");
    } finally {
      key?.fill(0);
    }
    renameSync(candidatePath, outputPath);
    const stat = ordinaryFile(outputPath, "output_file");
    if (stat.size > BigInt(Number.MAX_SAFE_INTEGER)) return invalid("output_size");
    return Object.freeze({
      format: PORTABLE_BACKUP_FORMAT,
...
export function restorePortableBackup(
  value: Readonly<RestorePortableBackupInput>,
): Readonly<PortableBackupResult> {
  ...
  if (backupStat.size > BigInt(MAX_BACKUP_BYTES)) return invalid("oversize");
  if (sha256File(backupPath) !== expected) return invalid("expected_sha256");
  ...
  if (secret.length !== 32) return invalid("authority_secret");
  ...
  try {
    ...
    if (existsSync(`${databasePath}-wal`) || existsSync(`${databasePath}-shm`)) return invalid("target_active");
    const previousExists = existsSync(databasePath);
    if (previousExists && !replaceExisting) return invalid("target_exists");
    ...
    if (movedPrevious && existsSync(previousPath)) renameSync(previousPath, databasePath);
    throw error;
  } finally {
