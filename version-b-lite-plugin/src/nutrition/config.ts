...
function descriptors(
  ...
  if (typeof value !== "object" || value === null) return invalid(`${path}:shape`);
  if (isProxy(value)) return invalid("proxy");
  if (Array.isArray(value) || Object.getPrototypeOf(value) !== Object.prototype) return invalid(`${path}:shape`);
  const keys = Reflect.ownKeys(value);
  if (keys.some((key) => typeof key !== "string" || !allowed.includes(key)) ||
      required.some((key) => !keys.includes(key))) return invalid(`${path}:keys`);
...
function cloneCredentialRefs(value: unknown): Readonly<Record<string, string>> {
  if (typeof value !== "object" || value === null) return invalid("credential_refs:shape");
  if (isProxy(value)) return invalid("proxy");
  if (Array.isArray(value) || Object.getPrototypeOf(value) !== Object.prototype) return invalid("credential_refs:shape");
  const keys = Reflect.ownKeys(value);
  const sourceIds = keys.map((key) => {
    if (typeof key !== "string" || REGISTERED_SOURCE_TIERS[key] === undefined) return invalid("credential_refs:key");
    return key;
  });
  ...
  for (const sourceId of sourceIds) {
    ...
    if (descriptor === undefined || !Object.hasOwn(descriptor, "value") || descriptor.enumerable !== true ||
        ...
        /[\u0000-\u001F\u007F]/u.test(descriptor.value)) return invalid(`credential_refs.${sourceId}`);
    copy[sourceId] = descriptor.value;
  }
  return Object.freeze(copy);
}

function cloneSources(value: unknown, refs: Readonly<Record<string, string>>): readonly Readonly<SourceRuntimeEntry>[] {
  if (typeof value !== "object" || value === null) return invalid("sources:shape");
  if (isProxy(value)) return invalid("proxy");
  if (!Array.isArray(value) || Object.getPrototypeOf(value) !== Array.prototype || value.length > 32) {
    return invalid("sources:shape");
  }
  const seen = new Set<string>();
  const entries = value.map((entry, index): SourceRuntimeEntry => {
    const data = descriptors(entry, ["source_id", "enabled", "backend_id", "backend_version"],
      ["source_id", "enabled", "backend_id", "backend_version"], `sources.${index}`);
...
export function cloneNutritionRuntimeConfig(value: unknown): Readonly<NutritionRuntimeConfig> {
  ...
  if (isProxy(value)) return invalid("proxy");
  const data = descriptors(value, ["policy_version", "resolution_deadline_ms", "sources", "credential_refs"],
    ["policy_version", "sources"], "config");
  ...
  if (typeof policyVersion !== "string" || policyVersion.length === 0 || policyVersion.length > 64 ||
      !Number.isInteger(deadline) || deadline < 500 || deadline > 5_000) return invalid("policy");
