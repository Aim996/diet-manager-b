...
interface PendingAuthority {
  readonly authority_kind: "diet-manager/nutrition-resolution/v6";
  readonly base_input_digest: string;
  readonly source_config_digest: string;
  readonly owner_nonce: string;
  readonly generation: number;
...
function exactKeys(value: Record<string, unknown>, expected: readonly string[], reason: string): void {
  ...
  if (keys.length !== sorted.length || keys.some((key, index) => key !== sorted[index])) invalid(reason);
}

function text(value: unknown, reason: string, max = 256): string {
...
function timestamp(value: unknown, reason: string): string {
  ...
  if (new Date(result).toISOString() !== result) return invalid(reason);
  return result;
}

function secret(value: Uint8Array): Buffer {
  if (!(value instanceof Uint8Array) || value.byteLength !== 32) return invalid("secret");
  return Buffer.from(value);
}

function mac(material: unknown, authoritySecret: Uint8Array): string {
  return createHmac("sha256", secret(authoritySecret)).update(canonicalJson(material), "utf8").digest("hex").toUpperCase();
}

function matchesMac(material: unknown, supplied: unknown, authoritySecret: Uint8Array): boolean {
  if (typeof supplied !== "string" || !DIGEST.test(supplied)) return false;
  const expected = Buffer.from(mac(material, authoritySecret), "hex");
  const actual = Buffer.from(supplied, "hex");
  return actual.length === expected.length && timingSafeEqual(actual, expected);
}

function pendingAuthority(input: ClaimInput, generation: number): Readonly<PendingAuthority> {
  if (!DIGEST.test(input.base_input_digest) || !DIGEST.test(input.source_config_digest)) invalid("digest");
  timestamp(input.now, "now");
  timestamp(input.lease_expires_at, "lease");
  if (Date.parse(input.lease_expires_at) <= Date.parse(input.now)) invalid("lease_order");
...
function pendingPayload(authority: Readonly<PendingAuthority>, authoritySecret: Uint8Array): string {
  return canonicalJson({ authority, authority_mac: mac(authority, authoritySecret) });
}

function parsePendingPayload(value: string, authoritySecret: Uint8Array): Readonly<PendingAuthority> {
  let parsed: unknown;
  try { parsed = JSON.parse(value); } catch { return invalid("pending_json"); }
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) return invalid("pending_shape");
  const wrapper = parsed as Record<string, unknown>;
  exactKeys(wrapper, ["authority", "authority_mac"], "pending_keys");
  const raw = wrapper.authority;
  if (typeof raw !== "object" || raw === null || Array.isArray(raw)) return invalid("pending_authority");
  ...
  if (authority.authority_kind !== "diet-manager/nutrition-resolution/v6" ||
      ...
      !Number.isSafeInteger(authority.generation) || (authority.generation as number) < 1) invalid("pending_authority_value");
  ...
  if (!matchesMac(authority, wrapper.authority_mac, authoritySecret) || canonicalJson(parsed) !== value) invalid("pending_mac");
  return freezeNutritionData(authority) as Readonly<PendingAuthority>;
}

export function parseNutritionPreviewMaterialV6(value: unknown): NutritionPreviewMaterialV6 {
  if (typeof value !== "object" || value === null || Array.isArray(value)) return invalid("material_shape");
  ...
  if (material.authority_kind !== "diet-manager/domain-preview/v6" ||
      ...
      !Array.isArray(material.effect_identities)) return invalid("material_value");
  ...
  const frozen = freezeNutritionData(material) as unknown as NutritionPreviewMaterialV6;
  if (canonicalSha256(frozen.nutrition_evidence) !== frozen.resolved_evidence_digest) invalid("evidence_digest");
  return frozen;
}

function finalPayload(material: NutritionPreviewMaterialV6, authoritySecret: Uint8Array): string {
...
function parseFinalPayload(value: string, authoritySecret: Uint8Array): NutritionPreviewMaterialV6 {
  let parsed: unknown;
  try { parsed = JSON.parse(value); } catch { return invalid("final_json"); }
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) return invalid("final_shape");
  const wrapper = parsed as Record<string, unknown>;
  exactKeys(wrapper, ["authority_kind", "material", "material_mac"], "final_keys");
  if (wrapper.authority_kind !== "diet-manager/nutrition-preview-store/v6") invalid("final_kind");
  const material = parseNutritionPreviewMaterialV6(wrapper.material);
  if (!matchesMac(material, wrapper.material_mac, authoritySecret) || canonicalJson(parsed) !== value) invalid("final_mac");
  return material;
}

function selectByIdempotency(database: DatabaseSync, idempotencyKey: string): ClaimRow | undefined {
  ...
  `).get(idempotencyKey) as unknown as ClaimRow | undefined;
}

function selectByEnvelope(database: DatabaseSync, envelopeId: string): ClaimRow | undefined {
  return database.prepare(`
    SELECT e.envelope_id, e.idempotency_key, e.input_digest AS envelope_input_digest,
      e.source_message_id, e.conversation_id, e.state AS envelope_state, e.result_status,
      e.payload_json, i.operation_id AS idempotency_operation_id,
    ...
    WHERE e.envelope_id = ?
  `).get(envelopeId) as unknown as ClaimRow | undefined;
}

function assertRowIdentity(row: ClaimRow): void {
  if (row.envelope_id !== row.idempotency_operation_id || row.envelope_input_digest !== row.idempotency_input_digest ||
      row.envelope_state !== "received") invalid("row_identity");
}

export function claimNutritionResolution(input: Readonly<ClaimInput>): NutritionClaimResult {
  const authority = pendingAuthority(input as ClaimInput, 1);
  ...
  try {
    ...
    if (row === undefined) {
      input.database.prepare(`INSERT INTO command_envelopes(
        ...
        .run(input.envelope_id, input.idempotency_key, input.base_input_digest, input.source_message_id,
          input.conversation_id, input.now, pendingPayload(authority, input.authority_secret));
    ...
    if (row.result_status === "preview_ready" && row.idempotency_state === "preview_ready") {
      ...
      if (material.base_input_digest !== input.base_input_digest) throw new Error("IDEMPOTENCY_CONFLICT:base_input_digest");
      if (material.operation_id !== input.operation_id || material.source_message_id !== input.source_message_id ||
          ...
          row.conversation_id !== input.conversation_id) throw new Error("IDEMPOTENCY_CONFLICT:binding");
      input.database.exec("ROLLBACK");
      open = false;
      return Object.freeze({ kind: "complete", envelope_id: row.envelope_id, material });
    }
    if (row.result_status !== "nutrition_resolving" || row.idempotency_state !== "nutrition_resolving") invalid("row_state");
    ...
    const next = pendingAuthority(input as ClaimInput, stored.generation + 1);
    ...
    if (updatedEnvelope.changes !== 1 || updatedIdempotency.changes !== 1) invalid("takeover_cas");
    input.database.exec("COMMIT");
    open = false;
    return Object.freeze({ kind: "owner", envelope_id: row.envelope_id, owner_nonce: input.owner_nonce, generation: next.generation });
  } catch (error) {
    if (open) try { input.database.exec("ROLLBACK"); } catch { /* preserve authority error */ }
    throw error;
  }
}

export function completeNutritionResolution(input: Readonly<CompleteClaimInput>): Readonly<{
  material: NutritionPreviewMaterialV6;
  won: boolean;
}> {
  timestamp(input.now, "complete_now");
  if (!Number.isSafeInteger(input.generation) || input.generation < 1) invalid("complete_generation");
  ...
  try {
    ...
    if (row === undefined) invalid("complete_missing");
    assertRowIdentity(row);
    if (row.result_status === "preview_ready" && row.idempotency_state === "preview_ready") {
    ...
    if (material.base_input_digest !== pending.base_input_digest ||
        ...
        material.conversation_id !== pending.conversation_id) invalid("complete_binding");
    ...
    if (changedEnvelope.changes !== 1 || changedIdempotency.changes !== 1) invalid("complete_cas");
    input.database.exec("COMMIT");
    open = false;
