...
export const TIME_RESOLVER_VERSION = "diet-manager/time-parser-v1" as const;
...
const LAST_NIGHT_PATTERN = /昨晚\s*(\d{1,2})\s*点/u;
...
function evidence(
  ...
  precision: OccurredTimeEvidence["precision"],
  resolutionBasis: OccurredTimeEvidence["resolution_basis"],
  anchor: OffsetIsoTimestamp,
): OccurredTimeEvidence {
...
export function resolveOccurredTime(
  sourceText: string,
  ...
  if (lastNight !== null) {
    ...
    if (statedHour >= 1 && statedHour <= 12) {
      const receivedShanghai = new Date(
        received.epoch_ms + SHANGHAI_OFFSET_MINUTES * MINUTE_MS,
      );
      const priorDate = new Date(
        Date.UTC(
          receivedShanghai.getUTCFullYear(),
          receivedShanghai.getUTCMonth(),
          receivedShanghai.getUTCDate(),
        ) - DAY_MS,
      );
