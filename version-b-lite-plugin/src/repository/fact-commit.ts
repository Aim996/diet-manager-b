...
export interface EnvelopeFactsResult {
  envelope_id: string;
  idempotency_key: string;
  input_digest: string;
  envelope_state: "effects_pending" | "effects_stable";
  result_status: "facts_committed_effects_pending" | "effects_stable";
...
function assertInventoryLocationCorrectionFact(
  database: DatabaseSync,
  event: FrozenFactEvent,
  commandType: DietManagerAction,
): void {
  if (event.eventType !== "inventory_adjusted") return;
  ...
  let payload;
  try {
    payload = validateAndFreezeInventoryLocationCorrectionFactPayload(JSON.parse(event.payloadJson) as unknown);
  } catch {
  ...
  const projection = database.prepare(
    `SELECT last_event_id, last_changed_at, payload_json
     FROM inventory_batch_projections WHERE batch_id = ?`,
  ).get(payload.batch_id) as {
    last_event_id: string;
    last_changed_at: string;
    payload_json: string;
  } | undefined;
  if (
    !projection || projection.last_event_id !== payload.previous_last_event_id ||
  ...
  for (const row of rows) {
    try {
      const existing = validateAndFreezeInventoryLocationCorrectionFactPayload(
        JSON.parse(row.payload_json) as unknown,
      );
      if (existing.batch_id === payload.batch_id) revision += 1;
    } catch {
...
function freezeOperation(value: PreparedEnvelopeOperation): FrozenEnvelopeOperation {
  ...
  if (!Number.isSafeInteger(fields.sequence.value) || (fields.sequence.value as number) < 0) {
  ...
  return Object.freeze({
    ...
    dataRevision: ascii(fields.dataRevision.value, "data_revision"),
    traceId: ascii(fields.traceId.value, "trace_id"),
    sequence: fields.sequence.value as number,
    operationId,
    event,
    items,
    effects,
...
function freezeSeal(value: PreparedEnvelopeSeal): FrozenEnvelopeSeal {
  const fields = exactDataProperties(value, SEAL_INPUT_FIELDS);
  const expectedOperationIds = denseArray(
    fields.expectedOperationIds.value,
    "expected_operation_ids",
    256,
  ).map((candidate) => ascii(candidate, "operation_id"));
  if (
...
function operationResultFor(
  input: FrozenEnvelopeOperation,
  envelopeId: string,
  idempotencyKey: string,
): OperationFactResult {
  return Object.freeze({
...
function orderedEnvelopeEvents(
  database: DatabaseSync,
  envelopeId: string,
): ExistingEventRow[] {
  return database
...
function assertOperationReplay(
  ...
  const effects = input.database
    ...
    .all(envelopeId, input.operationId) as unknown as ExistingEffectRow[];
  const expectedEffects = [...input.effects].sort((left, right) =>
    left.outboxId.localeCompare(right.outboxId, "en-US"),
  );
  if (
    ...
    effects.some((effect, index) => {
      const expected = expectedEffects[index];
      return (
        effect.outbox_id !== expected.outboxId ||
...
function parseTerminalBundleRevision(
  row: OperationBundleRow,
...
function parsePendingBundleRevision(
  row: OperationBundleRow,
  expectedSequence: number,
): string {
  let payload: unknown;
...
function assertOperationRevisionHandoff(
  input: FrozenEnvelopeOperation,
  envelopeId: string,
  ...
  if (events.length === 0) {
    if (currentRevision !== bindingRevision) throw new Error("PREVIEW_STALE:data_revision");
    const unexpectedBundles = changedCount(
      input.database,
  ...
  const bundles = input.database
    ...
    .all(envelopeId) as unknown as OperationBundleRow[];
  ...
  if (currentRevision !== expectedRevision) throw new Error("PREVIEW_STALE:data_revision");
...
function changedCount(
  ...
  const row = database.prepare(sql).get(value) as { count: number };
  return Number(row.count);
}

function insertOperationCheckpoint(
  input: FrozenEnvelopeOperation,
  envelopeId: string,
): void {
  ...
  input.database
    ...
    .run(
      envelopeId,
      input.operationId,
      terminal ? "succeeded" : "pending",
      terminal ? "applied" : "facts_committed_effects_pending",
      terminal ? input.event.committedAt : null,
      canonicalJson({
...
export function appendPreparedOperationFact(
  input: PreparedEnvelopeOperation,
  ...
  try {
    ...
    const result = operationResultFor(
      frozen,
      authority.binding.preview_id,
      authority.idempotency_key,
    );
...
export function appendPreparedOperationFactInOpenTransaction(
  input: PreparedEnvelopeOperation,
  options?: FactCommitOptions,
): OperationFactResult {
  const frozen = freezeOperation(input);
...
function authorizeOperationAuthority(
  frozen: FrozenEnvelopeOperation,
...
function appendOperationFacts(
  frozen: FrozenEnvelopeOperation,
  frozenOptions: Readonly<FactCommitOptions>,
  authority: AuthorizedRepositoryPreview,
): void {
  const events = orderedEnvelopeEvents(frozen.database, authority.binding.preview_id);
  assertOperationRevisionHandoff(
    frozen,
    authority.binding.preview_id,
...
export function sealPreparedEnvelopeFacts(input: PreparedEnvelopeSeal): EnvelopeFactsResult {
...
export function sealPreparedEnvelopeFactsInOpenTransaction(
  input: PreparedEnvelopeSeal,
): EnvelopeFactsResult {
  const frozen = freezeSeal(input);
  assertCurrentMigrationAuthority(frozen.database);
  ...
  if (
    operationIds.length !== frozen.expectedOperationIds.length ||
  ...
  const checkpointRows = frozen.database
    ...
    .all(authority.binding.preview_id) as unknown as OperationBundleRow[];
  if (checkpointRows.length !== events.length) {
    throw new Error("FACT_COMMIT_AUTHORITY_INVALID:bundle_count");
  }
  const checkpointByOperation = new Map(
    checkpointRows.map((row) => [row.operation_id, row]),
  );
  ...
  events.forEach((event, index) => {
    ...
    if (!checkpoint) throw new Error("FACT_COMMIT_AUTHORITY_INVALID:bundle_count");
  ...
  const outboxes = frozen.database
    ...
    .all(authority.binding.preview_id) as Array<{ operation_id: string; state: string }>;
  const allTerminal = outboxes.every(
    (row) => row.state === "succeeded" || row.state === "permanent_business_skip",
  );
  const bundleOperations = frozen.database
    .prepare(
      `SELECT operation_id FROM effect_bundle_commits
       WHERE envelope_id = ? AND completed_at IS NOT NULL
       ORDER BY operation_id`,
    )
    .all(authority.binding.preview_id) as Array<{ operation_id: string }>;
  const everyOperationBundled =
    bundleOperations.length === operationIds.length &&
    new Set(bundleOperations.map((row) => row.operation_id)).size === operationIds.length &&
    operationIds.every((operationId) =>
      bundleOperations.some((row) => row.operation_id === operationId),
    );
  ...
  if (authority.envelope_state === "received") {
    ...
    if (authority.envelope_state === "effects_pending" && stable) {
      ...
      authority.envelope_state !== (stable ? "effects_stable" : "effects_pending")
    ) {
