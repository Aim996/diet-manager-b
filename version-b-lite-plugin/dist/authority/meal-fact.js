...
function ordinaryClone(value, path, depth, context) {
    if (depth > MAX_DEPTH)
    ...
    try {
        if (Array.isArray(value)) {
            ...
            for (let index = 0; index < length; index += 1) {
                ...
                clone.push(ordinaryClone(descriptor.value, `${path}[${index}]`, depth + 1, context));
            }
...
function cloneMealFactJson(value, path) {
    return ordinaryClone(value, path, 0, { active: new WeakSet(), nodes: 0 });
}
...
function boundedText(value, path, maxLength) {
...
function safeNonnegativeInteger(value, path) {
...
function evidenceTimestamp(value, path) {
    ...
    if (year < 1_000 || year > 9_999 || month < 1 || month > 12 ||
...
function validateOccurredTime(value, path, occurredAt) {
    ...
    enumValue(evidence.resolver_version, ["diet-manager/time-parser-v1"], `${path}.resolver_version`);
    if (basis === "needs_clarification" || precision === "unknown" ||
...
export function validateAndFreezeOccurredTimeEvidence(value, options = {}) {
...
function validateSubject(value, path) {
    ...
    if (Object.hasOwn(subject, "excluded_non_self_share_count")) {
        const count = safeNonnegativeInteger(subject.excluded_non_self_share_count, `${path}.excluded_non_self_share_count`);
    ...
    if ((basis === "omitted_subject_default" && matchedSpan !== null) ||
...
function validateContextItem(value, path) {
    ...
    if (item.unit !== null)
        boundedText(item.unit, `${path}.unit`, MAX_UNIT_LENGTH);
}
function validateAcceptedContext(value, path) {
    if (typeof value !== "object" || value === null || Array.isArray(value)) {
        return invalid(`${path}:shape`);
    }
    const source = value;
    ...
    if (generated.epoch >= validUntil.epoch)
        return invalid(`${path}.valid_interval`);
    ...
    if (Object.hasOwn(context, "items")) {
        ...
        context.items.forEach((item, index) => validateContextItem(item, `${path}.items[${index}]`));
    }
    return Object.hasOwn(context, "scene")
        ? enumValue(context.scene, ["home", "outside", "company", "unknown"], `${path}.scene`)
        : "unknown";
}
function validateContext(value, path) {
    ...
    const scene = enumValue(context.scene, ["home", "outside", "company", "unknown"], `${path}.scene`);
    ...
    const expiredIds = context.expired_context_ids.map((id, index) => boundedText(id, `${path}.expired_context_ids[${index}]`, MAX_ID_LENGTH));
    if (new Set(expiredIds).size !== expiredIds.length) {
        return invalid(`${path}.expired_context_ids`);
    }
    if (typeof context.inventory_read !== "boolean")
        return invalid(`${path}.inventory_read`);
    const acceptedScene = context.accepted_context === null
        ? "unknown"
        : validateAcceptedContext(context.accepted_context, `${path}.accepted_context`);
    if (scene !== acceptedScene)
        return invalid(`${path}.scene`);
    enumValue(context.rule_version, ["diet-manager/context-v1"], `${path}.rule_version`);
}
export function optionalMealEvidenceFields(value) {
    return MEAL_EVIDENCE_OPTIONAL_FIELDS.filter((field) => Object.hasOwn(value, field));
}
export function validateMealOperationEvidence(value, occurredAt, path) {
...
export function validateAndFreezeMealFactPayload(value, options = {}) {
    ...
    enumValue(payload.authority_kind, ["diet-manager/meal-fact/v1"], `${path}.authority_kind`);
    ...
    if (Object.hasOwn(payload, "occurred_time")) {
        validateOccurredTime(payload.occurred_time, `${path}.occurred_time`, options.occurredAt);
    }
    if (Object.hasOwn(payload, "subject"))
        validateSubject(payload.subject, `${path}.subject`);
    if (Object.hasOwn(payload, "context"))
        validateContext(payload.context, `${path}.context`);
    return payload;
}
