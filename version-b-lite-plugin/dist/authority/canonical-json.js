...
function visit(value, depth, context) {
    ...
    try {
        ...
        return `{${stringKeys
            .sort()
            .map((key) => `${JSON.stringify(key)}:${visit(descriptors[key].value, depth + 1, context)}`)
            .join(",")}}`;
    }
    finally {
...
export function canonicalJson(value) {
    const result = visit(value, 0, { active: new WeakSet(), nodes: 0 });
    if (Buffer.byteLength(result, "utf8") > MAX_UTF8_BYTES)
