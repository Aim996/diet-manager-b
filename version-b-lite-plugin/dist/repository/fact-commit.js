...
function freezeEvent(value) {
    ...
    return Object.freeze({
        ...
        receivedAt: assertOffsetIsoTimestamp(fields.receivedAt.value, () => requestInvalid("received_at")),
...
function insertCorrectionFact(database, event, commandType) {
    ...
    let value;
    try {
        value = JSON.parse(event.payloadJson);
    }
    catch {
    ...
    if (Object.keys(payload).sort().join("\u0000") !== expectedFields.sort().join("\u0000") ||
    ...
    database.prepare(`INSERT INTO correction_events(
