...
const STATE_COEFFICIENT = {
    cut: 0.85,
    maintain: 1.0,
    bulk: 1.1,
};
const STATE_PROTEIN_PER_KG = {
    cut: 2.0,
    maintain: 1.6,
    bulk: 2.0,
};
export function deriveSixGoals(profile) {
    const height = positiveFinite(profile.height_cm, "height_cm");
    const weight = positiveFinite(profile.weight_kg, "weight_kg");
    const age = profile.age === null || profile.age === undefined
        ? DEFAULT_AGE
    ...
    const goalState = state === null ? "maintain" : state;
