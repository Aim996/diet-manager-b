...
function assertNonWriting(outcome) {
    if (outcome.status === "failed")
        throw new Error(`DIET_INSTALL_INIT_FAILED:${outcome.error_code}`);
    if (outcome.status !== "ignored")
        throw new Error(`DIET_INSTALL_INIT_FAILED:${outcome.status}`);
    return outcome;
}
function initializeOfficialDataRoot(root) {
    ...
    try {
        ...
        const businessRows = (meals.meal_history?.meals.length ?? 0) +
        ...
        if (businessRows !== 0)
            throw new Error("DIET_INSTALL_INIT_FAILED:non_empty");
        return {
...
function usage() {
    process.stderr.write("usage: backup <private-root> <backup-file> | " +
...
async function main(args) {
    ...
    if (operation === "backup") {
        ...
        if (extra.length !== 0 || privateRuntimeRoot === undefined || backupPath === undefined)
            usage();
    ...
    if (operation === "restore") {
        ...
        if (extra.length !== 0 || privateRuntimeRoot === undefined ||
            backupPath === undefined || expectedSha256 === undefined)
            usage();
    ...
    if (operation === "restore-portable") {
        ...
        if (extra.length !== 0 && !replaceExisting)
            usage();
