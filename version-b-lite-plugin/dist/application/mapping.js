...
function decimalMicrounits(value, scale, field) {
...
export function mapResolvedNutritionAmountMicrounits(evidence) {
    return evidence.adopted_amount === null
        ? null
        : decimalMicrounits(evidence.adopted_amount, 1000000n, "adopted_amount");
}
export function mapResolvedNutritionEvidenceToDomainSource(evidence) {
    if (evidence.source_type === "unknown")
        return null;
    const nutrient = (field, scale) => {
    ...
    return Object.freeze({
        ...
        basis_microunits: decimalMicrounits(evidence.basis_amount, 1000000n, "basis_amount"),
        basis_unit: evidence.basis_unit,
        nutrients: Object.freeze({
            energy_kcal_milli: nutrient("energy_kcal", 1000n),
            protein_mg: nutrient("protein_g", 1000n),
            fat_mg: nutrient("fat_g", 1000n),
            carbohydrate_mg: nutrient("carbohydrate_g", 1000n),
            fiber_mg: nutrient("fiber_g", 1000n),
            water_ml_milli: nutrient("water_ml", 1000n),
        }),
    });
}
function mealOrWaterOperation(command, nutritionEvidence = Object.freeze([])) {
...
function pantryEvidence(item, resolution, receivedAt, legacy) {
...
function purchaseAmount(item) {
    ...
    return Object.freeze({ unit: "unknown", observed_microunits: null, evidence: "unknown" });
}
function newPurchaseItem(command) {
    return Object.freeze({
        order: 0,
...
function purchaseOperations(request, command, resolutions) {
    ...
    return Object.freeze(items.map((item, index) => {
        ...
        return {
            ...
            nutrition_sources: [],
            pantry_evidence: pantryEvidence(item, resolution, request.received_at, "items" in command ? undefined : command),
        };
    }));
}
function locationCorrectionOperation(command, resolution) {
...
function nutritionSupplementOperation(command, resolution) {
    ...
    return Object.freeze({
        kind: "correct_record",
        operation_id: command.operation_id,
        correction_kind: "nutrition_supplement",
...
export function mapCoreCandidateToEnvelope(request, command, purchaseResolutions = Object.freeze([]), correctionResolution, nutritionEvidence = Object.freeze([])) {
...
export function mapRestoreCandidateToEnvelope(request, command, targetEventId, baseRevision) {
    const digest = identity(request);
    const operations = Object.freeze([Object.freeze({
            kind: "restore_record",
            operation_id: command.operation_id,
