export const CONTEXT_RULE_VERSION = "diet-manager/context-v1";
...
function hasRevisionIdentity(entry, input) {
...
function structurallyRelevant(entry, input) {
...
function targetOccurrenceEpoch(input, receivedEpoch) {
...
function isExplicitCorrection(sourceText) {
...
export function resolveMealContext(input) {
    const receivedEpoch = timestampEpoch(input.received_at);
    if (receivedEpoch === null)
        throw new Error("CORE_CONTEXT_INVALID:received_at");
    ...
    const latestRevisionBySource = new Map();
    for (const entry of input.prior_context) {
        if (!hasRevisionIdentity(entry, input))
            continue;
        latestRevisionBySource.set(entry.source_message_id, Math.max(latestRevisionBySource.get(entry.source_message_id) ?? 0, entry.revision));
    }
    const blockedLatestSources = new Set();
    const latestEntryCountBySource = new Map();
    for (const entry of input.prior_context) {
        if (!hasRevisionIdentity(entry, input))
            continue;
        if (entry.revision !== latestRevisionBySource.get(entry.source_message_id))
            continue;
        latestEntryCountBySource.set(entry.source_message_id, (latestEntryCountBySource.get(entry.source_message_id) ?? 0) + 1);
    ...
    for (const [sourceMessageId, count] of latestEntryCountBySource) {
        if (count !== 1)
            blockedLatestSources.add(sourceMessageId);
    }
    ...
    for (const entry of input.prior_context) {
        ...
        if (corrected || generated > receivedEpoch || generated >= validUntil)
            continue;
        candidates.push({ entry, generated });
    }
    candidates.sort((left, right) => right.generated - left.generated ||
