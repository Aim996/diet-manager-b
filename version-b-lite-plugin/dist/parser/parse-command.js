...
function classifyHealthIntent(sourceText) {
    ...
    for (const rawClause of sourceText.split(/(?:[，,。；;！？!?\r\n]+|然后|同时)/u)) {
        ...
        if (clause.length === 0)
            continue;
        if (HEALTH_ACTUAL_CLAUSE.test(clause))
            return "actual_request";
        if (HEALTH_EXPLANATION_CLAUSE.test(clause))
            explanation = true;
    }
...
function detachedFrozen(value) {
    if (Array.isArray(value)) {
        return Object.freeze(value.map((entry) => detachedFrozen(entry)));
    }
...
function shanghaiCalendarDate(timestamp, dayDelta) {
...
function dateAtShanghai1600(value) {
    if (value === null)
        return null;
...
function addShanghaiCalendarDays(date, dayDelta) {
...
function inventoryCandidate(input) {
    ...
    if (stockedAt === null || expirationAt === null)
        return null;
    return {
...
function ambiguityQuestion(input) {
...
function subjectEvidence(subject) {
    return {
        kind: subject.kind,
...
function retainedMealEntries(meal, completion) {
    const retained = [];
    for (let index = 0; index < meal.proposed_items.length; index += 1) {
        ...
        if (occurrence === undefined || item === undefined)
            continue;
        const excluded = completion.excluded_items.some((evidence) => evidence.normalized_name === occurrence.normalized_name &&
            occurrence.position >= evidence.matched_evidence.start &&
            occurrence.end <= evidence.matched_evidence.end);
        if (!excluded)
            retained.push({ occurrence, item });
    }
    return Object.freeze(retained);
}
function hasUnsafeMealLiquidAggregate(retained) {
    let knownMl = 0;
    for (const { item } of retained) {
        if (item.kind !== "nutritious_drink" || item.unit !== "ml" ||
            item.quantity === null)
            continue;
...
function mealCandidate(input, completion, occurredTime, meal, retained) {
    ...
    const command = {
        ...
        parser_version: PARSER_VERSION,
        occurred_time: occurredTime,
        subject: subjectEvidence(retained[0]?.occurrence.subject_evidence ?? meal.subject),
        items,
        ...
        ...(meal.group_amount_evidence === undefined || !retained.some(({ occurrence }) => occurrence.occurrence_id === meal.group_amount_evidence?.occurrence_id)
            ? {}
...
export function parseCoreCommand(value) {
    ...
    if (meal.occurrence_limit_exceeded ||
    ...
    if (retainedMeal.some(({ occurrence }) => occurrence.amount_resolution === "invalid") || hasUnsafeMealLiquidAggregate(retainedMeal)) {
        return detachedFrozen({
            disposition: "needs_clarification",
            action: "record_meal",
            reason_code: "amount_ambiguous",
    ...
    if (hasResolvedOccurrence &&
        meal.event_owners.some((owner) => owner.ownership === "unresolved" ||
    ...
    if (waterResolution.non_self_direct_count > 0 &&
