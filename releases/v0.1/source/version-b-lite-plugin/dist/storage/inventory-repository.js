...
import { productIdentityFingerprint, validateAndFreezeInventoryLocationCorrectionFactPayload, validateAndFreezePantryPurchaseEvidence, validateAndFreezeProductIdentityEvidence, } from "../domain/inventory-service.js";
import { buildPantryPurchaseReceiptItem } from "../domain/receipt.js";
import { deriveDomainId } from "../domain/identity.js";
import { assertAppliedInventoryLocationCorrectionAuthority, assertAuthenticatedInventoryLocationCorrectionFactAuthority, } from "../repository/inventory-location-correction-authority.js";
...
function nullableSafeNonnegativeInteger(value, reason) {
    if (value === null)
        return null;
...
function parsedJson(value, reason) {
    let parsed;
    try {
    ...
    if (canonicalJson(parsed) !== value)
        return invalid(`${reason}_canonical`);
    return parsed;
}
...
export function preparePantryPurchase(input) {
    ...
    if (input.quantity_microunits !== null)
        positiveSafeInteger(input.quantity_microunits, "quantity_microunits");
...
export function createPantryProjectionPayload(input) {
...
export function parseProductPayload(value) {
    ...
    const record = exact(value, ["authority_kind", "identity", "identity_fingerprint"], "product_payload");
    if (record.authority_kind !== "diet-manager/product/v2")
        return invalid("product_payload_kind");
    ...
    if (productIdentityFingerprint(identity) !== fingerprint)
        return invalid("product_identity_fingerprint");
    return Object.freeze({
        version: 2,
...
export function parseBatchPayload(value) {
    ...
    const record = exact(value, ["authority_kind", "pantry_evidence", "template_reference_microunits"], "batch_payload");
...
export function parseBatchPayloadJson(value) {
    return parseBatchPayload(parsedJson(value, "batch_payload"));
}
export function parseProjectionPayload(value) {
    const hasEvidence = typeof value === "object" && value !== null && !Array.isArray(value) &&
        Object.hasOwn(value, "pantry_evidence");
    ...
    if (record.authority_kind === "diet-manager/inventory-projection/v1" && !hasEvidence) {
        if (quantity === null)
            return invalid("projection_quantity");
...
export function parseProjectionPayloadJson(value) {
    return parseProjectionPayload(parsedJson(value, "projection_payload"));
}
export function parsePurchaseFactPayload(value) {
    const hasEvidence = typeof value === "object" && value !== null && !Array.isArray(value) &&
        Object.hasOwn(value, "pantry_evidence");
    ...
    if (typeof record.effect_inputs !== "object" || record.effect_inputs === null ||
        Array.isArray(record.effect_inputs))
        return invalid("purchase_effect_inputs");
    const effects = exact(record.effect_inputs, Object.keys(record.effect_inputs), "purchase_effect_inputs");
...
export function parsePurchaseFactPayloadJson(value) {
    return parsePurchaseFactPayload(parsedJson(value, "purchase_fact_payload"));
}
export function parseInventoryAllocationPlan(value) {
    ...
    const allocations = record.allocations.map((candidate, index) => {
        ...
        if (before === null || after === null || before - deducted !== after ||
    ...
    const issueByStatus = Object.freeze({
        matched: null,
    ...
    if (issue !== issueByStatus[record.status])
        return invalid("inventory_allocation_issue");
    const noRead = record.status === "skipped_outside" || record.status === "skipped_by_user" ||
        record.status === "skipped_amount_unknown";
    if (record.read_required !== !noRead)
        return invalid("inventory_allocation_read_required");
...
export function pendingMealInventoryAllocationBatchIds(database) {
    const rows = database.prepare(`SELECT i.payload_json, o.state AS outbox_state
     ...
     ORDER BY e.committed_at, e.event_id, i.item_order`).all();
    ...
    for (const row of rows) {
        ...
        for (const allocation of plan.allocations)
            batchIds.add(allocation.batch_id);
    }
    return batchIds;
}
export function assertAuthenticatedPantryPurchaseRoot(database, authoritySecret, batchId) {
    const initialRow = database.prepare(`SELECT b.stock_event_id, b.product_id, b.schema_version AS batch_schema_version,
     ...
     WHERE b.batch_id = ?`).get(batchId);
    if (initialRow === undefined)
        return invalid("purchase_event_identity");
    let initialEvidence;
    try {
        const authenticated = assertAuthenticatedPurchaseEventAuthority(database, authoritySecret, initialRow.stock_event_id);
        ...
        const outboxes = database.prepare(`SELECT outbox_id,envelope_id,operation_id,effect_id,effect_kind,previous_state,
              state,attempt_count,reason,created_at,updated_at
       FROM effect_outbox
       WHERE envelope_id = ? AND operation_id = ? ORDER BY effect_id`).all(authenticated.envelope_id, authenticated.operation_id);
        const transactions = database.prepare(`SELECT transaction_id,event_id,product_id,batch_id,idempotency_key,schema_version,
       ...
       FROM inventory_transactions WHERE event_id = ? ORDER BY transaction_id`).all(authenticated.event_id);
        const bundles = database.prepare(`SELECT stage,effect_state,result_status,completed_at,payload_json
       FROM effect_bundle_commits WHERE envelope_id = ? AND operation_id = ?`).all(authenticated.envelope_id, authenticated.operation_id);
        ...
        if (product.version !== 2 || product.identity === null || batch.version !== 2 ||
...
export function assertCurrentInventoryLocationCorrectionLineage(database, authoritySecret, batchId, currentEvidence) {
    const initialEvidence = assertAuthenticatedPantryPurchaseRoot(database, authoritySecret, batchId);
    const rows = database.prepare(`SELECT e.event_id, e.envelope_id, e.operation_id, e.payload_json,
    ...
    for (const row of rows) {
        try {
            const entry = Object.freeze({
                row,
                payload: assertAuthenticatedInventoryLocationCorrectionFactAuthority(database, authoritySecret, row.envelope_id, row.operation_id, row.event_id),
            });
    ...
    authenticatedRows.sort((left, right) => left.payload.base_revision !== right.payload.base_revision
    ...
    for (const { row, payload } of authenticatedRows) {
        try {
            assertAppliedInventoryLocationCorrectionAuthority(database, row.envelope_id, row.operation_id, row.event_id, { requireCurrentProjection: false });
        }
        catch (error) {
            if (error instanceof Error &&
                error.message.startsWith("INVENTORY_LOCATION_CORRECTION_AUTHORITY_INVALID:"))
...
export function pendingLocationCorrectionBatchIds(database) {
    const rows = database.prepare(`SELECT e.payload_json
     FROM event_records e
     JOIN command_envelopes c ON c.envelope_id = e.envelope_id
     WHERE e.event_type = 'inventory_adjusted' AND e.fact_kind = 'inventory'
...
export function listPantryAllocationCandidates(database, authoritySecret, normalizedName, occurredAt) {
    safeText(normalizedName, "inventory_candidate_name");
    const occurredEpoch = Date.parse(occurredAt);
    if (!Number.isFinite(occurredEpoch))
        return invalid("inventory_candidate_time");
    const rows = database.prepare(`SELECT p.product_id, p.payload_json AS product_payload_json,
     ...
     ORDER BY b.batch_id`).all(normalizedName);
    if (rows.length === 0)
        return null;
    ...
    for (const row of rows) {
        ...
        if (product.version !== 2 || batch.version !== 2 || projection.version !== 2)
            return null;
        if (product.identity_fingerprint === null || batch.pantry_evidence === null ||
        ...
        assertCurrentInventoryLocationCorrectionLineage(database, authoritySecret, row.batch_id, projection.pantry_evidence);
...
export function applyPantryAllocationsInTransaction(input) {
    const plan = parseInventoryAllocationPlan(input.plan);
    if (plan.status !== "matched")
        return Object.freeze([]);
    const occurredEpoch = Date.parse(input.occurred_at);
    if (!Number.isFinite(occurredEpoch))
        return invalid("inventory_allocation_time");
    const existing = input.database.prepare(`SELECT transaction_id,event_id,product_id,batch_id,idempotency_key,schema_version,
            direction,reason_code,unit,related_event_id,related_transaction_id,
            source_message_id,conversation_id,received_at,committed_at,result_status,
            lifecycle_status,payload_json
     FROM inventory_transactions
     WHERE event_id = ? AND idempotency_key = ? AND direction = 'out'`).all(input.event_id, input.effect_id);
    if (existing.length !== 0) {
        if (existing.length !== plan.allocations.length)
            return invalid("inventory_allocation_replay");
        ...
        const replayed = plan.allocations.map((allocation, index) => {
            ...
            if (!row || row.event_id !== input.event_id || row.product_id !== allocation.product_id ||
                ...
                row.payload_json !== expectedPayload)
                return invalid("inventory_allocation_replay");
    ...
    const applied = plan.allocations.map((allocation, index) => {
        const row = input.database.prepare(`SELECT i.quantity_status, i.effective_status, i.payload_json,
              b.payload_json AS batch_payload_json
       FROM inventory_batch_projections i
       JOIN inventory_batches b ON b.batch_id = i.batch_id
       WHERE i.batch_id = ?`).get(allocation.batch_id);
        ...
        if (projection.version !== 2 || projection.pantry_evidence === null ||
            ...
            return invalid("inventory_allocation_projection");
        assertCurrentInventoryLocationCorrectionLineage(input.database, input.authority_secret, allocation.batch_id, projection.pantry_evidence);
        const expiration = projection.pantry_evidence.expiration.effective_at;
        ...
        NULL, ?, ?, ?, ?, 'applied', 'active', ?)`).run(transactionId, input.event_id, allocation.product_id, allocation.batch_id, input.effect_id, allocation.unit, input.source_message_id, input.conversation_id, input.received_at, input.committed_at, canonicalJson({
            allocation_index: index,
            authority_kind: "diet-manager/inventory-transaction/v2",
            quantity_after_microunits: allocation.after_microunits,
            quantity_delta_microunits: -allocation.deducted_microunits,
        ...
        input.database.prepare(`UPDATE inventory_batch_projections
       SET last_event_id = ?, last_changed_at = ?, quantity_status = ?, effective_status = ?, payload_json = ?
       WHERE batch_id = ? AND quantity_status = 'available' AND effective_status = 'active' AND payload_json = ?`).run(input.event_id, input.committed_at, allocation.after_microunits === 0 ? "empty" : "available", allocation.after_microunits === 0 ? "empty" : "active", canonicalJson(nextPayload), allocation.batch_id, row.payload_json);
        if (Number(input.database.prepare("SELECT changes() AS count").get().count) !== 1) {
...
export function assertStoredPurchaseFactMatchesExpected(input) {
    const { expectedFact } = input;
    if (expectedFact.database !== input.database || expectedFact.operationId !== input.operationId ||
    ...
    try {
       ...
       FROM event_records WHERE envelope_id = ? AND operation_id = ?`).all(input.envelopeId, input.operationId);
        ...
        if (input.requireAppliedEffect === false) {
            const itemCount = input.database.prepare("SELECT COUNT(*) AS count FROM meal_items WHERE event_id = ?").get(event.event_id);
            const outboxes = input.database.prepare(`SELECT outbox_id,effect_id,effect_kind,previous_state,reason
         FROM effect_outbox WHERE envelope_id = ? AND operation_id = ? ORDER BY outbox_id`).all(input.envelopeId, input.operationId);
        ...
        if (effect.kind !== "inventory_add" || effect.reason_code !== "purchase" ||
            typeof product.product_id !== "string" || typeof batch.batch_id !== "string")
            throw new Error("PURCHASE_EFFECT_AUTHORITY_INVALID:terminal_expected_fact");
        const storedProduct = input.database.prepare("SELECT brand,payload_json FROM products WHERE product_id = ?").get(product.product_id);
        const storedBatch = input.database.prepare(`SELECT product_id,explicit_expiration_at,quantity_unit,payload_json
       FROM inventory_batches WHERE batch_id = ?`).get(batch.batch_id);
        const storedProjection = input.database.prepare("SELECT payload_json FROM inventory_batch_projections WHERE batch_id = ?").get(batch.batch_id);
        ...
        try {
            ...
            if (storedProduct.brand !== (expectedIdentity?.brand ?? null))
                throw new Error("product_brand");
        ...
        if (projectionPayload.version !== expectedPayload.version ||
            ...
            throw new Error("PURCHASE_EFFECT_AUTHORITY_INVALID:terminal_business_payload");
        const transactions = input.database.prepare(`SELECT transaction_id,event_id,product_id,batch_id,direction,reason_code,unit,
              result_status,lifecycle_status,payload_json
       FROM inventory_transactions WHERE event_id = ? ORDER BY transaction_id`).all(event.event_id);
        const transaction = transactions[0];
