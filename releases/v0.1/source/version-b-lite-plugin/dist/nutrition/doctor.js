import { freezeNutritionData, REGISTERED_SOURCE_TIERS, } from "./types.js";
function healthAction(health, reason) {
    if (health === "ok")
        return null;
    if (reason === "source_disabled")
        return "enable_source_if_authorized";
    if (reason === "adapter_missing")
        return "install_registered_backend";
    if (reason === "credential_missing")
        return "configure_credential_reference";
    if (health === "timeout")
        return "check_source_availability";
    return "check_backend_configuration";
}
function probeWithAbort(promise, signal) {
    if (signal.aborted)
        return Promise.resolve(undefined);
    return new Promise((resolve) => {
        const onAbort = () => resolve(undefined);
        signal.addEventListener("abort", onAbort, { once: true });
        promise.then((value) => { signal.removeEventListener("abort", onAbort); resolve(value); }, () => { signal.removeEventListener("abort", onAbort); resolve(undefined); });
    });
}
export async function runNutritionDoctor(config, adapters, options = {}) {
