...
function stableProfilePayload(value, retrievedAt, createdAt) {
    let parsed;
    try {
    ...
    if (canonicalJson(parsed) !== value || typeof parsed !== "object" || parsed === null || Array.isArray(parsed))
        return null;
    ...
    if (!Number.isFinite(timestamp.valueOf()) || timestamp.toISOString() !== retrievedAt)
        return null;
...
function profileMatches(row, profile) {
...
function insertProfile(database, profile) {
    const existing = database.prepare("SELECT * FROM nutrition_profiles WHERE nutrition_profile_id = ?")
        .get(profile.nutrition_profile_id);
    if (existing !== undefined) {
        if (!profileMatches(existing, profile))
            invalid("profile_conflict");
        return;
    }
    ...
    source_version,retrieved_at,coverage_status,created_at,supersedes_profile_id,payload_json
        ...
        .run(profile.nutrition_profile_id, profile.schema_version, profile.subject_type, profile.subject_id, profile.profile_version, profile.source_type, profile.source_ref, profile.source_version, profile.retrieved_at, profile.coverage_status, profile.created_at, profile.supersedes_profile_id, canonicalJson(profile));
}
function insertSnapshot(database, snapshot) {
    ...
    if (existing !== undefined) {
        if (!snapshotMatches(existing, snapshot))
            invalid("snapshot_conflict");
        return;
    }
    ...
    source_type,source_ref,coverage_status,created_at,payload_json
        ...
        .run(snapshot.snapshot_id, snapshot.schema_version, snapshot.meal_event_id, snapshot.intake_item_id, snapshot.nutrition_profile_id, snapshot.profile_version, snapshot.source_type, snapshot.source_ref, snapshot.coverage_status, snapshot.created_at, canonicalJson(snapshot));
}
export function persistNutritionRecords(database, records) {
...
export function assertNutritionRecordsPersisted(database, records) {
    for (const record of records) {
        const profile = database.prepare("SELECT * FROM nutrition_profiles WHERE nutrition_profile_id = ?")
            .get(record.profile.nutrition_profile_id);
        const snapshot = database.prepare("SELECT * FROM nutrition_snapshots WHERE snapshot_id = ?")
            .get(record.snapshot.snapshot_id);
        if (profile === undefined || !profileMatches(profile, record.profile))
            invalid("profile_readback");
        if (snapshot === undefined || !snapshotMatches(snapshot, record.snapshot))
            invalid("snapshot_readback");
    }
}
export function readNutritionRecordsForMeal(database, mealEventId) {
    ...
    WHERE s.meal_event_id = ? ORDER BY s.intake_item_id, s.snapshot_id`).all(mealEventId);
