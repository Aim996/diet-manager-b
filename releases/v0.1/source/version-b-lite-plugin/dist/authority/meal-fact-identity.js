...
function text(value, reason, maxLength = 256) {
...
function sequence(value, reason) {
...
function payloadWithoutReservation(value, occurredAt) {
    const payload = validateAndFreezeMealFactPayload(value, { occurredAt });
    return Object.freeze(Object.fromEntries(Object.entries(payload).filter(([key]) => key !== "progress_reservation")));
}
export function createMealFactIdentity(input) {
...
function parseItemIdentity(value, expectedOrder) {
    const item = exactRecord(value, [
        "item_id", "item_order", "item_type", "normalized_name_digest", "payload_digest",
    ], "item");
    if (sequence(item.item_order, "item_order") !== expectedOrder)
        return invalid("item_order");
    return Object.freeze({
        item_id: text(item.item_id, "item_id"),
...
export function parseMealFactPreviewMaterial(value) {
    ...
    if (!Array.isArray(material.meal_fact_identities) ||
    ...
    if (new Set(identities.map((identity) => identity.sequence)).size !== identities.length ||
...
export function mealFactIdentityEquals(left, right) {
