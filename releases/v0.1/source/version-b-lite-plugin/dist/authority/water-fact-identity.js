...
function invalid(reason) { throw new TypeError(`WATER_FACT_IDENTITY_INVALID:${reason}`); }
...
export function createWaterFactIdentity(input) {
    ...
    return Object.freeze({
        ...
        meal_id: null, meal_slot: null, payload_digest: canonicalSha256(payload), items: Object.freeze([]),
    });
}
export function waterFactIdentityEquals(left, right) {
    return canonicalJson(left) === canonicalJson(right);
}
export function parseWaterFactPreviewMaterial(value) {
    ...
    if (Object.keys(record).sort().join("\0") !== fields.sort().join("\0") || record.authority_kind !== "diet-manager/domain-preview/v3" ||
        ...
        !Array.isArray(record.water_fact_identities) || record.water_fact_identities.length !== 1)
        return invalid("material");
    const identity = record.water_fact_identities[0];
    ...
    if (typeof identity !== "object" || identity === null || Array.isArray(identity) ||
        ...
        !Number.isSafeInteger(identity.sequence) || identity.sequence < 0 || identity.schema_version !== "domain/v2" ||
        identity.event_type !== "diet_water" || identity.fact_kind !== "water" || identity.meal_id !== null || identity.meal_slot !== null ||
        !DIGEST.test(String(identity.payload_digest)) || !Array.isArray(identity.items) || identity.items.length !== 0)
        return invalid("identity");
    const frozenIdentity = Object.freeze({
        sequence: identity.sequence, event_id: text(identity.event_id, "event_id"), operation_id: text(identity.operation_id, "operation_id"),
        schema_version: "domain/v2", event_type: "diet_water", fact_kind: "water",
        ...
        meal_id: null, meal_slot: null, payload_digest: String(identity.payload_digest), items: Object.freeze([]),
    });
