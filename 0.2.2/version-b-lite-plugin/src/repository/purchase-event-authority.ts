...
export interface AuthenticatedPurchaseEvent {
  ...
  readonly envelope_event_count: number;
  readonly payload_json: string;
}

interface StoredPurchaseEventRow {
  event_id: string;
  envelope_id: string;
  operation_id: string;
...
export function assertAuthenticatedPurchaseEventAuthority(
  database: DatabaseSync,
  authoritySecret: Uint8Array,
  eventId: string,
): Readonly<AuthenticatedPurchaseEvent> {
  const rows = database.prepare(
    `SELECT e.event_id, e.envelope_id, e.operation_id, e.schema_version,
     ...
     WHERE e.event_id = ?`,
  ).all(eventId) as unknown as Array<StoredPurchaseEventRow & {
    preview_payload_json: string;
    idempotency_key: string;
    input_digest: string;
    envelope_received_at: string;
    envelope_state: string;
  }>;
  const row = rows[0];
  if (
    rows.length !== 1 || row === undefined || row.event_id !== eventId ||
  ...
  const storedEnvelopeIds = database.prepare(
  ...
  if (
    storedEnvelopeIds.length !== expectedEnvelopeIds.length ||
  ...
  const storedRows = database.prepare(
    `SELECT event_id,envelope_id,operation_id,schema_version,event_type,fact_kind,
     ...
     ORDER BY event_id`,
  ).all(row.envelope_id) as unknown as StoredPurchaseEventRow[];
  ...
  if (
    storedRows.length !== expectedIdentities.length ||
  ...
  for (let index = 0; index < storedRows.length; index += 1) {
    ...
    if (
      stored === undefined || expected === undefined || stored.envelope_id !== row.envelope_id ||
      ...
      stored.committed_at !== new Date(
        Date.parse(material.committed_at_base) + expected.sequence,
      ).toISOString()
    ) return invalid("identity");
    let payload: unknown;
    try {
      payload = JSON.parse(stored.payload_json) as unknown;
      if (canonicalJson(payload) !== stored.payload_json) return invalid("payload");
    } catch {
    ...
    if (!purchaseFactIdentityEquals(actual, expected)) return invalid("identity");
  }
  const storedMeals = database.prepare(
    `SELECT event_id,envelope_id,operation_id,schema_version,event_type,fact_kind,
            source_message_id,conversation_id,received_at,committed_at,occurred_at_text,
            meal_id,meal_slot,payload_json
     FROM event_records
  ...
  const expectedMeals = [...material.meal_fact_identities]
    .sort((left, right) => left.event_id < right.event_id ? -1 : left.event_id > right.event_id ? 1 : 0);
  if (
    storedMeals.length !== expectedMeals.length ||
  ...
  for (let index = 0; index < storedMeals.length; index += 1) {
    ...
    try {
      itemInputs = items.map((item) => {
        const itemPayload = JSON.parse(item.payload_json) as unknown;
        if (canonicalJson(itemPayload) !== item.payload_json) return invalid("meal_item");
    ...
    if (!mealFactIdentityEquals(actual, expected)) return invalid("identity");
  }
  const expected = expectedIdentities.find((identity) =>
    identity.event_id === row.event_id && identity.operation_id === row.operation_id);
  if (expected === undefined) return invalid("identity");
  return Object.freeze({
