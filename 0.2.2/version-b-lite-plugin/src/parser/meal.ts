...
function resolveAmountEvidence(
  ...
  return occurrenceAmount(
    explicitAmount(matchedSpan.trim(), quantity, unit),
    "resolved",
  );
}
...
function amountForOccurrence(
  ...
  if (beforeMatch !== null) {
    return resolveAmountEvidence(
      beforeMatch[1] ?? "", beforeMatch[2] ?? "", beforeMatch[0], item, frame,
    );
  }
  const bareClassifier = BARE_CLASSIFIER_BEFORE_ITEM.exec(before);
  if (bareClassifier !== null) {
    return resolveAmountEvidence(
      "一",
      bareClassifier[1] ?? "",
      bareClassifier[0],
  ...
  if (afterMatch !== null) {
    return resolveAmountEvidence(
      afterMatch[1] ?? "", afterMatch[2] ?? "", afterMatch[0], item, frame,
    );
  }
