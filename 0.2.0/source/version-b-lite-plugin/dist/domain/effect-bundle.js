...
import { createReplacementProgressReservation, reservationFromEventPayload, reservationsFromEventPayload, } from "../repository/progress-reservation.js";
...
import { deriveSixGoals, emptyConfiguredGoals, GOAL_FIELDS, mergeGoalOverrides, } from "./goal-derivation.js";
...
function makeReplacementReservation(database, progressDate, beforeNutrients, afterNutrients) {
...
export function prepareCorrectionOperation(input) {
    ...
    if (operationKind === "change_time") {
        ...
        const mealNutrition = preflightCorrectionNutrition(input.database, current.snapshot, current.snapshot, allItemOrders).before;
    ...
    try {
       ...
       WHERE envelope_id = ? AND operation_id = ? ORDER BY effect_id`).all(input.envelopeId, input.operationId);
        ...
        for (let index = 0; index < progressReplacementCount; index += 1) {
            expectedKinds.set(deriveDomainId("effect", input.idempotencyKey, compensationEffectCount + index), "daily_progress_replacement");
        }
       ...
       FROM correction_events WHERE request_id = ?`).get(input.operationId);
        ...
        const reservations = reservationsFromEventPayload(payload, correctionEvent.event_type);
       ...
       WHERE e.event_id = ? AND e.event_type = 'diet_meal'`).get(correction.target_event_id);
        ...
        const dailyProgressByDate = reservations.map((reservation) => Object.freeze({
       ...
       WHERE envelope_id = ? AND operation_id = ? AND effect_state = 'pending'`).run(hasIssues ? "permanent_business_skip" : "succeeded", hasIssues ? "applied_with_issues" : "applied", input.now, canonicalJson({
            ...
            effects: terminalEffects.map((effect) => {
                ...
                const reservationIndex = reservations.findIndex((reservation, index) => deriveDomainId("effect", input.idempotencyKey, compensationEffectCount + index) ===
                    effect.effect_id);
...
export function readAppliedCorrectionResult(input) {
    ...
    const outboxes = input.database.prepare(`SELECT effect_id, effect_kind, state FROM effect_outbox
     WHERE envelope_id = ? AND operation_id = ? ORDER BY effect_id`).all(input.envelopeId, input.operationId);
    if (outboxes.length !== bundle.effects.length ||
    ...
    const replacementByEffectId = new Map();
    ...
    const progressByDate = reservations.map((_reservation, reservationIndex) => {
        const effectId = deriveDomainId("effect", input.idempotencyKey, compensationEffectCount + reservationIndex);
...
const PROFILE_FACT_AUTHORITY_KIND = "diet-manager/profile-fact/v1";
const PROFILE_PAYLOAD_AUTHORITY_KIND = "diet-manager/profile/v1";
const GOAL_VERSION_PAYLOAD_AUTHORITY_KIND = "diet-manager/goal-version/v1";
const PROFILE_EFFECT_KIND = "profile_effect";
function profileEffectError(reason) {
    throw new Error(`PROFILE_EFFECT_AUTHORITY_INVALID:${reason}`);
}
function profileParseCanonical(value, label) {
    let parsed;
    try {
        parsed = JSON.parse(value);
    }
...
function profileExactKeys(value, fields, label) {
    if (Object.keys(value).sort().join(" ") !== [...fields].sort().join(" ")) {
...
function profileGoalState(value) {
    if (value === null)
        return null;
...
export function prepareProfileOperation(input) {
    const { operation } = input;
    if (operation.kind !== "set_profile")
        return invalid("profile_operation");
...
export function assertStoredProfileFactMatchesExpected(input) {
    ...
    try {
       ...
       FROM event_records WHERE envelope_id = ? AND operation_id = ?`).get(input.envelopeId, input.operationId);
        ...
        const items = input.database.prepare("SELECT COUNT(*) AS count FROM meal_items WHERE event_id = ?").get(expected.eventId);
        if (items.count !== 0)
            return profileEffectError("stored_items");
        input.database.exec("ROLLBACK");
        transactionOpen = false;
    }
    catch (error) {
...
function profileIntentFromStoredFact(database, envelopeId, operationId, idempotencyKey) {
    const event = database.prepare(`SELECT event_type, fact_kind, meal_id, meal_slot, payload_json
     FROM event_records WHERE envelope_id = ? AND operation_id = ?`).get(envelopeId, operationId);
    if (!event || event.event_type !== "diet_profile" || event.fact_kind !== "profile" ||
    ...
    if (payload.authority_kind !== PROFILE_FACT_AUTHORITY_KIND)
        return profileEffectError("payload");
    const effectInputs = payload.effect_inputs;
    const effectId = deriveDomainId("effect", idempotencyKey, 0);
    const intent = effectInputs[effectId];
    ...
    if (intent.kind !== "profile_apply" || intent.timezone !== "Asia/Shanghai")
        return profileEffectError("effect_input");
    if (typeof intent.profile_id !== "string" || typeof intent.goal_version_id !== "string" ||
        typeof intent.user_id !== "string")
        return profileEffectError("effect_input");
    if (!Number.isFinite(intent.height_cm) || intent.height_cm <= 0 ||
        !Number.isFinite(intent.weight_kg) || intent.weight_kg <= 0)
        return profileEffectError("effect_input");
    const sex = profileSex(intent.sex);
    const goalState = profileGoalState(intent.goal_state);
    if (intent.age !== null &&
        (!Number.isInteger(intent.age) || intent.age <= 0))
        return profileEffectError("effect_input");
    const storedGoals = intent.goals;
    ...
    if (canonicalJson(storedGoals) !== canonicalJson(expectedGoals))
        return profileEffectError("goals");
    return {
        profileId: intent.profile_id,
        goalVersionId: intent.goal_version_id,
        userId: intent.user_id,
        heightCm: intent.height_cm,
        weightKg: intent.weight_kg,
        sex,
        age: intent.age,
        goalState,
        goals: expectedGoals,
...
function assertPendingProfileAuthority(input) {
    const checkpoint = input.database.prepare(`SELECT effect_state, result_status, completed_at, payload_json FROM effect_bundle_commits
     WHERE envelope_id = ? AND operation_id = ?`).get(input.envelopeId, input.operationId);
    if (!checkpoint || checkpoint.effect_state !== "pending" || checkpoint.result_status !== "facts_committed_effects_pending" || checkpoint.completed_at !== null) {
    ...
    if (typeof expected !== "object" || expected === null || Array.isArray(expected))
        return profileEffectError("checkpoint");
    profileExactKeys(expected, ["effect_id", "state"], "profile_checkpoint");
    const outboxes = input.database.prepare(`SELECT outbox_id, effect_id, effect_kind, state FROM effect_outbox WHERE envelope_id = ? AND operation_id = ?`).all(input.envelopeId, input.operationId);
    if (outboxes.length !== 1 || outboxes[0]?.outbox_id !== outboxId || outboxes[0].effect_id !== effectId ||
...
function profileResult(input, intent) {
    return Object.freeze({
        sequence: input.operationSequence,
        operation_id: input.operationId,
        status: "committed",
...
export function applyProfileEffect(input) {
    let transactionOpen = false;
    try {
        input.database.exec("BEGIN IMMEDIATE");
        transactionOpen = true;
       ...
       AND state = ?`).run(input.now, input.envelopeId, input.operationId, outbox.state);
        if (changed(input.database) !== 1)
            return profileEffectError("claim");
        input.database.prepare(`INSERT INTO user_profiles(
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, ?)`).run(intent.profileId, intent.userId, "domain/v2", intent.heightCm, intent.weightKg, intent.sex, intent.age, intent.goalState, input.now, input.now, canonicalJson({
        ...
        input.database.prepare(`INSERT INTO goal_versions(
        goal_version_id, schema_version, user_id, timezone, effective_from, effective_to, created_at, payload_json
      ) VALUES (?, ?, ?, ?, ?, NULL, ?, ?)`).run(intent.goalVersionId, "domain/v2", intent.userId, "Asia/Shanghai", input.now, input.now, canonicalJson({
       ...
       WHERE envelope_id = ? AND operation_id = ? AND state = 'processing'`).run(input.now, input.envelopeId, input.operationId);
        if (changed(input.database) !== 1)
            return profileEffectError("complete");
        input.database.prepare(`UPDATE effect_bundle_commits SET effect_state = 'succeeded', result_status = 'applied',
       completed_at = ?, payload_json = ? WHERE envelope_id = ? AND operation_id = ?
       AND effect_state = 'pending'`).run(input.now, canonicalJson({
            authority_kind: "diet-manager/effect-bundle/v1",
        ...
        if (changed(input.database) !== 1)
            return profileEffectError("bundle");
        input.database.exec("COMMIT");
        transactionOpen = false;
        return profileResult(input, intent);
    }
...
const GOAL_FACT_AUTHORITY_KIND = "diet-manager/goal-fact/v1";
const GOAL_EFFECT_KIND = "goal_effect";
function goalEffectError(reason) {
    throw new Error(`GOAL_EFFECT_AUTHORITY_INVALID:${reason}`);
}
function goalParseCanonical(value, label) {
    let parsed;
    try {
        parsed = JSON.parse(value);
    }
...
function goalExactKeys(value, fields, label) {
    if (Object.keys(value).sort().join(" ") !== [...fields].sort().join(" ")) {
        goalEffectError(`${label}_keys`);
    }
}
function goalPositive(value, label) {
    if (value === null)
        return null;
    if (!Number.isFinite(value) || value <= 0)
        return goalEffectError(label);
    return value;
}
function parseGoalOverrides(value, label) {
    if (typeof value !== "object" || value === null || Array.isArray(value))
        return goalEffectError(label);
    const record = value;
    const present = Object.keys(record);
    if (present.length === 0 || present.some((key) => !GOAL_FIELDS.includes(key))) {
        return goalEffectError(`${label}_keys`);
    }
    const result = Object.create(null);
    for (const field of present)
        result[field] = goalPositive(record[field], `${label}.${field}`);
    return Object.freeze(result);
}
function parseConfiguredGoals(value, label) {
    if (typeof value !== "object" || value === null || Array.isArray(value))
        return goalEffectError(label);
    const record = value;
...
export function prepareGoalOperation(input) {
    const { operation } = input;
    if (operation.kind !== "set_goal")
        return invalid("goal_operation");
    const goalVersionId = deriveDomainId("goal", input.idempotencyKey, input.sequence);
    const eventId = deriveDomainId("event", input.idempotencyKey, input.sequence);
    const effectId = deriveDomainId("effect", input.idempotencyKey, input.sequence);
    const outboxId = deriveDomainId("outbox", input.idempotencyKey, input.sequence);
...
export function assertStoredGoalFactMatchesExpected(input) {
    ...
    try {
       ...
       FROM event_records WHERE envelope_id = ? AND operation_id = ?`).get(input.envelopeId, input.operationId);
        ...
        const items = input.database.prepare("SELECT COUNT(*) AS count FROM meal_items WHERE event_id = ?").get(expected.eventId);
        if (items.count !== 0)
            return goalEffectError("stored_items");
        input.database.exec("ROLLBACK");
        transactionOpen = false;
    }
    catch (error) {
...
function goalIntentFromStoredFact(database, envelopeId, operationId, idempotencyKey) {
    const event = database.prepare(`SELECT event_type, fact_kind, meal_id, meal_slot, payload_json
     FROM event_records WHERE envelope_id = ? AND operation_id = ?`).get(envelopeId, operationId);
    if (!event || event.event_type !== "diet_goal" || event.fact_kind !== "goal" ||
        event.meal_id !== null || event.meal_slot !== null)
    ...
    if (payload.authority_kind !== GOAL_FACT_AUTHORITY_KIND)
        return goalEffectError("payload");
    const effectInputs = payload.effect_inputs;
    const effectId = deriveDomainId("effect", idempotencyKey, 0);
    const intent = effectInputs[effectId];
    if (typeof intent !== "object" || intent === null || Array.isArray(intent))
        return goalEffectError("effect_input");
    goalExactKeys(intent, ["kind", "goal_version_id", "user_id", "timezone", "goals"], "goal_effect_input");
    if (intent.kind !== "goal_apply" || intent.timezone !== "Asia/Shanghai")
        return goalEffectError("effect_input");
    if (typeof intent.goal_version_id !== "string" || typeof intent.user_id !== "string")
        return goalEffectError("effect_input");
...
export function currentConfiguredGoals(database, userId) {
    ...
    if (payload.authority_kind !== GOAL_VERSION_PAYLOAD_AUTHORITY_KIND)
        return goalEffectError("goal_version");
    return parseConfiguredGoals(payload.goals, "goal_version_goals");
}
function assertPendingGoalAuthority(input) {
    const checkpoint = input.database.prepare(`SELECT effect_state, result_status, completed_at, payload_json FROM effect_bundle_commits
     WHERE envelope_id = ? AND operation_id = ?`).get(input.envelopeId, input.operationId);
    if (!checkpoint || checkpoint.effect_state !== "pending" || checkpoint.result_status !== "facts_committed_effects_pending" || checkpoint.completed_at !== null) {
    ...
    if (typeof expected !== "object" || expected === null || Array.isArray(expected))
        return goalEffectError("checkpoint");
    goalExactKeys(expected, ["effect_id", "state"], "goal_checkpoint");
    const outboxes = input.database.prepare(`SELECT outbox_id, effect_id, effect_kind, state FROM effect_outbox WHERE envelope_id = ? AND operation_id = ?`).all(input.envelopeId, input.operationId);
    if (outboxes.length !== 1 || outboxes[0]?.outbox_id !== outboxId || outboxes[0].effect_id !== effectId ||
...
function goalResult(input, intent, goals) {
    return Object.freeze({
        sequence: input.operationSequence,
        operation_id: input.operationId,
        status: "committed",
...
export function applyGoalEffect(input) {
    let transactionOpen = false;
    try {
        input.database.exec("BEGIN IMMEDIATE");
        transactionOpen = true;
       ...
       AND state = ?`).run(input.now, input.envelopeId, input.operationId, outbox.state);
        if (changed(input.database) !== 1)
            return goalEffectError("claim");
        const merged = mergeGoalOverrides(currentConfiguredGoals(input.database, intent.userId), intent.overrides);
        input.database.prepare(`UPDATE goal_versions SET effective_to = ? WHERE user_id = ? AND effective_to IS NULL AND effective_from < ?`).run(input.now, intent.userId, input.now);
        input.database.prepare(`INSERT INTO goal_versions(
        goal_version_id, schema_version, user_id, timezone, effective_from, effective_to, created_at, payload_json
      ) VALUES (?, ?, ?, ?, ?, NULL, ?, ?)`).run(intent.goalVersionId, "domain/v2", intent.userId, "Asia/Shanghai", input.now, input.now, canonicalJson({
            authority_kind: GOAL_VERSION_PAYLOAD_AUTHORITY_KIND,
       ...
       WHERE envelope_id = ? AND operation_id = ? AND state = 'processing'`).run(input.now, input.envelopeId, input.operationId);
        if (changed(input.database) !== 1)
            return goalEffectError("complete");
        input.database.prepare(`UPDATE effect_bundle_commits SET effect_state = 'succeeded', result_status = 'applied',
       completed_at = ?, payload_json = ? WHERE envelope_id = ? AND operation_id = ?
       AND effect_state = 'pending'`).run(input.now, canonicalJson({
            authority_kind: "diet-manager/effect-bundle/v1",
