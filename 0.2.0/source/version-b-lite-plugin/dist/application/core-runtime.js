...
import { chmodSync, closeSync, constants, fstatSync, fsyncSync, linkSync, lstatSync, openSync, readSync, renameSync, unlinkSync, writeSync, } from "node:fs";
...
import { computeGoalProgressBars } from "../domain/goal-derivation.js";
...
import { mapResolvedNutritionAmountMicrounits, mapResolvedNutritionEvidenceToDomainSource, mapCoreCandidateToEnvelope, mapUndoCandidateToEnvelope, mapRestoreCandidateToEnvelope, } from "./mapping.js";
...
function writeAuthoritySecretFile(authority, bytes, replace) {
    assertRuntimeRootAuthority(authority);
    if (bytes.length !== 32)
        return invalid("secret", "length");
...
export function installAuthoritySecret(root, bytes) {
    if (bytes.length !== 32)
        return invalid("secret", "length");
...
function readDailyProgress(session, request) {
    ...
    const configuredGoals = currentConfiguredGoals(session.database, "user:self");
...
export function handleCoreRequest(runtime, value) {
    ...
    try {
        ...
        if (parsed.command.action === "restore_record") {
            ...
            if (resolvedTarget.active) {
                return nonWritingOutcome(request.action, request.operation_id, "ignored", "already_active");
            }
            const envelope = mapRestoreCandidateToEnvelope(request, parsed.command, resolvedTarget.target_event_id, resolvedTarget.base_revision);
            const preview = session.service.preview(envelope);
            let result;
            try {
                result = session.service.execute({
