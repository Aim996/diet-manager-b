...
async function boundedJson(response) {
    ...
    if (contentType !== "application/json")
        throw new TypeError("NUTRITION_SOURCE_INVALID:content_type");
...
function emptyResolution(status, reason) {
...
function parseFood(value) {
    const root = ordinaryRecord(value);
    const foods = root?.foods;
    if (!Array.isArray(foods) || foods.length === 0)
        return emptyResolution("no_results", "no_match");
    ...
    for (const candidate of food.foodNutrients) {
        ...
        if (binding === undefined || nutrient.unitName.toUpperCase() !== binding.unit ||
            nutrientValues[binding.field] !== null)
            continue;
    ...
    const complete = ["energy_kcal", "protein_g", "fat_g", "carbohydrate_g", "fiber_g"]
        .every((field) => nutrientValues[field] !== null);
    const evidence = {
        source_id: "public.usda_fooddata_central",
...
export class FoodDataCentralHttpTransport {
    #fetch;
    constructor(fetchImplementation = fetch) {
        if (typeof fetchImplementation !== "function")
            throw new TypeError("NUTRITION_TRANSPORT_INVALID:fetch");
    ...
    async resolve(input) {
        ...
        if (!response.ok)
            return emptyResolution("error", "source_unavailable");
